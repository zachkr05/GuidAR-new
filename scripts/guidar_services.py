...
class GuidARServices:
    def __init__(self):
        ...
        self.traj_pub = rospy.Publisher(
            "/server/TransferTrajectory", JointTrajectory, queue_size=1, latch=True
        )
...
#        for cls, obj_id in class_to_id.items():
    ...
    def _get_obstacles(self,):
        ...
        vicon_topics = [topic_name for topic_name, topic_type in all_topics if f"/vicon/{VICON_TOPIC_PREFIX}" in topic_name]
    ...
    def _generate_costmap_and_path(self, goal_world):
        ...
        sigma = 5
        x_px = gaussian_filter1d(path_arr[:, 1].astype(float), sigma)
        y_px = gaussian_filter1d(path_arr[:, 0].astype(float), sigma)
